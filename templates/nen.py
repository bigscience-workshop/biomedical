...
class YourNENDatasetName(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, data_dir, split):
        ...
        if self.config.name == _DATASETNAME:
            ...
            for idx, article_id in enumerate(article_ids):
                example = self._build_example(data_dir=data_dir, article_id=article_id)
                yield (idx, example.to_dict())

    @abstractmethod
    def _build_example(self, data_dir: str, article_id: str) -> Example:
        """
        Load example from data directory
    ...
    def _get_article_ids_from_split(self, data_dir: str, split: str) -> Iterable[str]:
        """
        Retrive article ids for the given split
