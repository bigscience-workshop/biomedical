...
class N2C22008ObesityDataset(datasets.GeneratorBasedBuilder):
            ...
            features = datasets.Features(
                {
                    ...
                    "textual": [datasets.Value("string")],
                    "intuitive": [datasets.Value("string")],
                }
            )
    ...
    def _get_source_sample(sample):
        textual_labels = [
            disease_name + "_" + sample["textual"][disease_name]
            for disease_name in sample["textual"].keys()
            if sample["textual"][disease_name]
        ]
        intuitive_labels = [
            disease_name + "_" + sample["intuitive"][disease_name]
        ...
        return {
            ...
            "text": sample["text"],
            "textual": textual_labels,
            "intuitive": intuitive_labels,
        }

    @staticmethod
    def _get_bigbio_sample(sample):
        labels = [
            task + "_" + disease_name + "_" + sample[task][disease_name]
            for task in ["textual", "intuitive"]
            for disease_name in sample[task].keys()
            if sample[task][disease_name]
        ]
        return {
        ...
        if split == "train":
            ...
            for sample in samples:
                if self.config.schema == "source":
                    ...
                    yield _id, self._get_bigbio_sample(sample)
                _id += 1
